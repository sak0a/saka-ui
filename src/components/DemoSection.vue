...
<template>
  <div class="bg-(--s-bg-secondary) rounded-2xl border border-(--s-border) transition-all duration-300">
    <!-- Header with title and actions -->
    <div class="flex items-start justify-between p-5 pb-0">
      <div class="flex-1">
        <div class="text-lg font-semibold text-(--s-text-primary) mb-1">{{ title }}</div>
        <p v-if="description" class="text-sm text-(--s-text-secondary) mb-4">{{ description }}</p>
      </div>
      
      <!-- Action buttons -->
      <div class="flex items-center gap-2 ml-4">
        <button 
          class="p-2 text-(--s-text-secondary) hover:text-(--s-text-primary) hover:bg-(--s-bg-tertiary) rounded-lg transition-colors"
        ...
        <button 
          @click="toggleCode"
          :class="isCodeVisible ? 'bg-(--s-bg-tertiary) text-(--s-text-primary)' : 'text-(--s-text-secondary) hover:text-(--s-text-primary) hover:bg-(--s-bg-tertiary)'"
    ...
    <div v-if="hasSlotContent && !codeOnly" class="p-5">
      <div :class="[
        ...
        customBackground || 'bg-(--s-bg-primary)'
      ]">
        <slot />
      </div>
    </div>
    ...
    >
      <div v-show="isCodeVisible" class="border-t border-(--s-border) overflow-hidden transition-colors duration-300">
        <div class="bg-(--s-bg-tertiary) p-6 overflow-x-auto">
